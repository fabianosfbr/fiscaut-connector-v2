...
class DatabaseLogHandler(logging.Handler):
    def __init__(self, level=logging.NOTSET):
        ...
        if logging_service is None:  # Apenas para log inicial se falhou de cara
            try:
                ...
                logging.info(
                    "DatabaseLogHandler: logging_service carregado na inicialização."
                )
                self.service_checked_at_init = True
            except Exception as e:
                logging.warning(
                    f"DatabaseLogHandler: Falha inicial ao tentar carregar logging_service: {e}. Tentará novamente no primeiro emit."
                )
        ...
        if logging_service is None:
            try:
                ...
                from .services.logging_service import (
                    logging_service as service_instance,
                )

                logging_service = service_instance  # Atualiza a variável global
                logging.info(
                    "DatabaseLogHandler: logging_service carregado dinamicamente com sucesso."
                )
                ...
                logging.critical(
                    f"DatabaseLogHandler: Falha crítica ao carregar logging_service dinamicamente: {e}. O handler será desabilitado.",
                    exc_info=True,
                )
    ...
    def emit(self, record: logging.LogRecord):
        ...
        if logging_service is None:
            if (
                not self._ensure_service()
            ):  # Tenta carregar e verifica se foi desabilitado
        ...
        try:
            ...
            message = self.format(
                record
            )  # Pega a mensagem formatada pelo Formatter do handler
            ...
            if record.exc_info:
                ...
                tb = "".join(
                    tb
                )  # Converter a lista de strings do traceback em uma única string
            elif (
            ...
            log = logging.getLogger(
                "DatabaseLogHandlerInternal"
            )  # Usar um logger específico para erros do handler
            ...
            log.error(
                f"LogRecord que falhou: Level={record.levelname}, Module={record.module}, Message (raw)={record.getMessage()[:200]}..."
            )
